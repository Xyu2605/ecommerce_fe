...
export const productService = {
    ...
    getProductsByName : async(productName : string) : Promise<IAPIResponse<IProduct[]>> => {
        const response = await axiosInstance.post(endpoints.PRODUCT_ENDPOINTS.GET_PRODUCT_BY_NAME(productName));
        return response.data;
    },
    getProductsByBrand : async(brand : string) : Promise<IAPIResponse<IProduct[]>> => {
        const response = await axiosInstance.post(endpoints.PRODUCT_ENDPOINTS.GET_PRODUCT_BY_BRAND(brand));
        return response.data;
    },
    getProductById : async(productId : number) : Promise<IAPIResponse<IProduct>> => {
        const response = await axiosInstance.post(endpoints.PRODUCT_ENDPOINTS.GET_PRODUCT_BY_ID(productId));
        return response.data;
    },
    findProductByBrandAndName : async(brand : string, name : string) : Promise<IAPIResponse<IProduct[]>> => {
        const response  = await axiosInstance.post(endpoints.PRODUCT_ENDPOINTS.GET_PRODUCT_BY_BRAND_AND_NAME(brand, name));
        return response.data;
    },
    findProductsByCategoryAndBrand : async(category : string, brand : string) : Promise<IAPIResponse<IProduct[]>> => {
        const response = await axiosInstance.post(endpoints.PRODUCT_ENDPOINTS.GET_PRODUCT_BY_CATEGORY_AND_BRAND(category, brand));
        return response.data;
    },
